...
$(document).ready(function () {
  ...
  function buildSearchAndMountHistory() {
    ...
    searchHistory.forEach((city) => {
      ...
      li.className =
        "list list-group-item col-sm-10 col-md-10 col-lg-10 col-xl-10";
      li.innerHTML = city;
  ...
  async function performSearch(cityName) {
    ...
    const waveHeight =
      "Height: " +
      forecastResult.data.weather[0].hourly[2].swellHeight_ft +
      " Ft";
    $("#wave-height").text(waveHeight);
    const windspeed =
      "Speed: " +
      forecastResult.data.weather[0].hourly[2].windspeedMiles +
      " MPH";
    $("#wind-speed").text(windspeed);
    const windDir =
      "Direction: " + forecastResult.data.weather[0].hourly[2].swellDir16Point;
    $("#windDir").text(windDir);
    const temp =
      "Temperature: " + forecastResult.data.weather[0].hourly[2].tempF + " °F";
    $("#temp").text(temp);
